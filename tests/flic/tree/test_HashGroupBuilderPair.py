...
class HashGroupBuilderPairTest(unittest.TestCase):
    ...
    def test_two_builder_segmented(self):
        ...
        name_ctx = NameConstructorContext(
            manifest_schema_impl=SegmentedSchemaImpl(nc_id=NcId(3), schema=SegmentedSchema(manifest_prefix), tree_options=options),
            data_schema_impl=SegmentedSchemaImpl(nc_id=NcId(4), schema=SegmentedSchema(data_prefix), tree_options=options)
        )
